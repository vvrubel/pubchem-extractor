...
def test_input_spec():
    domain = "compound"
    namespace = "cid"
    identifiers = 2244  # input
    res = sync_req.input_specification(domain, namespace, identifiers)
...
def test_build_url():
    ...
    res = sync_req.build_url(input_spec, operation_spec)
    assert res == EXAMPLE
...
def test_request_data():
    ...
    params = {}
    res = sync_req.request_data(url, **params)
    expectation = {
        'PropertyTable': {
            'Properties': [
                {
...
def test_build_identifiers(inp, expect):
    res = sync_req.build_identifiers(*inp)
    assert res == expect
