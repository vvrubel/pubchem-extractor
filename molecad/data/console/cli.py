...
from molecad.data.db import create_index, upload_data
...
@click.group(
    help="Консольная утилита для извлечения информации о свойствах молекул с серверов Pubchem, "
         "сохранения полученных данных в файлы формата JSON и их загрузки в базу MongoDB."
)
def pubchem_to_db():
    pass
...
@click.option(
    "--test",
    is_flag=True,
    help="Если опция '--test' указана, то данные из файла будут загружены в тестовую коллекцию, "
         "иначе – в основную.",
)
def populate(f_dir: pathlib.Path, test: bool) -> None:
    if test:
        collection, schema = setup.get_test_collection
    else:
    ...
    for file in f_dir.iterdir():
        ...
        succeed += s
        failed += f
    click.echo(f"Succeed: {succeed},\n Failed: {failed}")


pubchem_to_db.add_command(fetch)
pubchem_to_db.add_command(populate)
pubchem_to_db.add_command(split)
