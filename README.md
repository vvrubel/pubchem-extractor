...
* По причине того что поиск будет производиться по smiles молекул, каждый документ должен 
...
* При загрузке в локальную базу данных, извлеченных из Pubchem, первичный индекс "_id" документов 
  ...
  Во избежание подобных ситуаций – дублирования документов – необходимо создать на коллекции 
  новый уникальный индекс, который привязан к уникальному полю документа. В нашем случае им 
  является поле "CID". 
