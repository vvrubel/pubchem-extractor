...
def input_specification(
    ...
    """
    This function builds the first part of URL setting up input identifiers.
    :param domain: string refers to database - compound | substance | assay
    :param namespace: string can assign values depending on the domain.
    #TODO make description
    if domain == `compound`:
    if domain == `substance`:
    if domain == `assay`:
    :param identifiers:
    :return: string is the first part of URL and formatted
...
def operation_specification(
    ...
    """
    The function dictates what to do with the input records –
    what information about them you want to retrieve.
    :param operation: string describes action to execute with input identifiers.
    :param property_tags: string are passed if only `operation == "property"`
...
def build_url(input_spec: str, operation_spec: str, output_format: str) -> str:
    """
    The command generates the URL string needed to request data from PubChem’s
    PUG REST service. URL structure consists of three pre-build parts
    assigned as parameters when invoking the function.
    # TODO
    :param input_spec: string represented as
    "<domain>/<namespace>/<identifiers>"
    :param operation_spec: string
    :param output_format: takes a value from the
    list = ["JSON", "XML", "SDF", "CSV", "PNG", "TXT"]
